...
class Experiment_discrete:
    ...
    def loop_1(self, goal, maze): #pairnaw to maze gia na kanw update ta frames sthn discrete
        ...
        for i_episode in range(1, self.max_episodes + 1):
            ...
            for timestep in range(1, self.max_timesteps + 1):
                ...
                duration_pause, _, _, _, _ = self.getKeyboard(actions, duration_pause, observation, timedout, reset)
                reset = False
    ...
    def getKeyboard(self, actions, duration_pause, observation_, timedout, reset):
        #kkkkkkkkkkkkkkkk
        reward = 0
        done = False
        action = self.get_action_pair()
        ...
        if reset:
            observation_, reward, done = self.env.step(action, timedout, "left_down",
                                                       reset, self.config['Experiment']['loop_1']['action_duration'])
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                ...
                if event.key in self.env.keys:
                    ...
                    # Environment step
                    observation_, reward, done = self.env.step(action, timedout, "left_down",
                                                            False, self.config['Experiment']['loop_1']['action_duration'])
